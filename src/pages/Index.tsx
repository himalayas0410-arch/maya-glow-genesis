...
import { Award, ArrowRight, GraduationCap, Heart, Shield, Star, Sparkles, Stethoscope, Scissors } from "lucide-react";
...
import workshopPosterImg from "@/assets/workshop-poster.jpg";
...
import beforeAfterBrowsImg from "@/assets/before-after-brows.jpg";
import beforeAfterEyesImg from "@/assets/before-after-eyes.jpg";
import clinicReceptionImg from "@/assets/clinic-reception.jpg";
import clinicProductZoneImg from "@/assets/clinic-product-zone.jpg";
...
const featuredTreatments = [
  { title: "HydraFacial", benefit: "Instant hydration and visible glow", img: facialImg },
  { title: "Botox", benefit: "Smooth lines with natural expression", img: injectImg },
  { title: "Hair Restoration", benefit: "Targeted scalp and density support", img: hairImg },
  { title: "Dimple Creation", benefit: "Quick enhancement, minimal downtime", img: dimpleImg },
  { title: "Vampire Facial", benefit: "Collagen stimulation and texture renewal", img: vampireImg },
  { title: "Fibroblasting", benefit: "Non-surgical tightening and resurfacing", img: fibroImg },
];

const beforeAfter = [
  { title: "Under Eye Filler", img: beforeAfterUnderEyeImg },
  { title: "Brow Enhancement", img: beforeAfterBrowsImg },
  { title: "Eye Rejuvenation", img: beforeAfterEyesImg },
];
...
const Index = () => (
  <Layout>
    ...
    {/* Results + Story (Image Left, Text Right) */}
    <section className="section-padding max-w-7xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
        <div className="rounded-2xl overflow-hidden shadow-elevated group">
          <img src={beforeAfterEyesImg} alt="Before and after aesthetic treatment result" loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
        </div>
        <div>
          <span className="text-sm tracking-[0.2em] uppercase text-primary font-semibold mb-3 block">Real Transformations</span>
          <h2 className="font-heading text-3xl md:text-4xl font-semibold text-foreground mb-4">From concern to confidence</h2>
          <p className="text-muted-foreground mb-6">Clinical precision, premium technology, and naturally beautiful outcomes.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-8">
            {["Natural-looking results", "Minimal downtime options", "Research-backed protocols", "Personalized treatment plan"].map((point) => (
          ...
          <a href="https://wa.me/919827947023?text=Hi%2C%20I%20want%20to%20see%20before-after%20results%20and%20book%20consultation" target="_blank" rel="noopener noreferrer" className="bg-gradient-gold text-primary-foreground px-8 py-3 rounded-full text-sm font-semibold hover:opacity-90 transition-opacity shadow-gold inline-block">
            See More Results
          </a>
        </div>
      </div>
    ...
    <section className="section-padding max-w-7xl mx-auto">
      ...
      <div className="text-center mt-10">
        <Link to="/pricing" className="inline-flex items-center gap-2 bg-foreground text-primary-foreground px-6 py-3 rounded-full text-sm font-semibold hover:opacity-90 transition-opacity">
          View Plans & Pricing <ArrowRight className="w-4 h-4" />
        </Link>
      </div>
    </section>

    {/* Featured Treatments (Text Left, Image Right + Cards) */}
    <section className="bg-secondary section-padding">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        <div>
          <span className="text-sm tracking-[0.2em] uppercase text-primary font-semibold mb-3 block">Featured Treatments</span>
          <h2 className="font-heading text-3xl md:text-4xl font-semibold text-foreground mb-4">Image-first treatment discovery</h2>
          <p className="text-muted-foreground mb-7">Explore our most requested treatments with clear outcomes and one-line benefits.</p>
          <a href="https://wa.me/919827947023?text=Hi%2C%20I%20need%20help%20choosing%20the%20right%20treatment" target="_blank" rel="noopener noreferrer" className="bg-gradient-gold text-primary-foreground px-8 py-3 rounded-full text-sm font-semibold hover:opacity-90 transition-opacity shadow-gold inline-block">
            Book Consultation
          </a>
        </div>
        <div className="rounded-2xl overflow-hidden shadow-elevated">
          <img src={dimpleImg} alt="Dimple creation treatment poster" loading="lazy" className="w-full h-full object-cover" />
        </div>
      </div>
      <div className="max-w-7xl mx-auto mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {featuredTreatments.map((t) => (
          <div key={t.title} className="group rounded-2xl overflow-hidden bg-card shadow-card hover:shadow-elevated transition-all duration-500 hover:-translate-y-1">
            <div className="aspect-[4/3] overflow-hidden">
              <img src={t.img} alt={t.title} loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
            </div>
            <div className="p-5">
              <h3 className="font-heading text-xl font-semibold text-foreground mb-2">{t.title}</h3>
    ...
    {/* Doctor + Trust (Image Left, Text Right) */}
    <section className="section-padding max-w-7xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <div>
          ...
          <p className="text-sm text-primary font-medium mb-4">Associate Professor · Internationally Trained · Facial Aesthetics & ENT Expert</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
            {["12+ Years Experience", "Associate Professor", "Research-Based Treatments", "International Fellowship Mentor"].map((item) => (
    ...
    {/* Academy (Text Left, Image Right) */}
    <section className="bg-secondary section-padding">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        <div>
          <span className="text-sm tracking-[0.2em] uppercase text-primary font-semibold mb-3 block">Aesthetic Academy</span>
          <h2 className="font-heading text-3xl md:text-4xl font-semibold text-foreground mb-4">Hands-on Fellowship Program</h2>
          <p className="text-muted-foreground mb-6">Train with live demonstrations, practical sessions, and certification-focused mentorship.</p>
          <div className="bg-card border border-border rounded-2xl p-5 mb-6">
            <p className="text-sm text-muted-foreground mb-1">Program Fee</p>
            <p className="font-heading text-3xl text-foreground">₹1,80,000</p>
            <p className="text-sm text-primary font-medium mt-2">Limited Seats · Next Batch Filling Fast</p>
          </div>
          <Link to="/academy" className="bg-gradient-gold text-primary-foreground px-8 py-3 rounded-full text-sm font-semibold hover:opacity-90 transition-opacity shadow-gold inline-block">
            Apply for Fellowship
          </Link>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <img src={workshopPosterImg} alt="Skincare workshop poster" loading="lazy" className="rounded-xl shadow-card w-full" />
          <img src={fellowshipCertImg} alt="Fellowship certification moment" loading="lazy" className="rounded-xl shadow-card w-full" />
          <img src={awardStageImg} alt="Award and conference recognition" loading="lazy" className="rounded-xl shadow-card w-full" />
          <img src={teamGroupImg} alt="Team and participants at clinic" loading="lazy" className="rounded-xl shadow-card w-full" />
        </div>
      </div>
    </section>

    {/* Clinic Interior + Gallery (Image Left, Text Right) */}
    <section className="section-padding max-w-7xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        <div className="grid grid-cols-2 gap-4">
          <img src={clinicImg} alt="Maya clinic interior" loading="lazy" className="rounded-xl shadow-card w-full h-full object-cover" />
          <img src={clinicReceptionImg} alt="Clinic reception area" loading="lazy" className="rounded-xl shadow-card w-full h-full object-cover" />
          <img src={clinicProductZoneImg} alt="Clinic product and consultation zone" loading="lazy" className="rounded-xl shadow-card w-full h-full object-cover" />
          <img src={clinicProcedureRoomImg} alt="Procedure room at clinic" loading="lazy" className="rounded-xl shadow-card w-full h-full object-cover" />
        </div>
        <div>
          <span className="text-sm tracking-[0.2em] uppercase text-primary font-semibold mb-3 block">Clinic Experience</span>
          <h2 className="font-heading text-3xl md:text-4xl font-semibold text-foreground mb-4">Premium ambience, clinical precision</h2>
          <p className="text-muted-foreground mb-6">A calm, modern environment designed for safety, privacy, and comfort in every visit.</p>
          <div className="space-y-3 mb-8">
            {["Advanced devices and technology", "Sterile and protocol-driven practice", "Private consultation-first approach", "Comfort-focused treatment rooms"].map((item) => (
    ...
    <section className="bg-secondary section-padding">
      <div className="max-w-7xl mx-auto">
        <SectionHeading subtitle="Before & After" title="Visible outcomes, real cases" description="Browse authentic treatment outcomes from procedures performed under medical supervision." />
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {beforeAfter.map((item) => (
            <div key={item.title} className="bg-card rounded-2xl overflow-hidden shadow-card hover:shadow-elevated transition-shadow">
